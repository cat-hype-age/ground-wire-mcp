"""Ground Wire External Data Server — MCP server for CPI-U and exchange rates.

Serves data that agents need but isn't in the Treasury Bulletins:
  - CPI-U (U.S. City Average, 1982-84=100, Not Seasonally Adjusted)
  - Historical exchange rates (USD/GBP, USD/DEM, USD/INR, USD/JPY, USD/CAD)

Tools:
  - lookup_cpi: Get CPI-U value for a year (and optional month)
  - lookup_exchange_rate: Get exchange rate for a currency pair and year
  - inflation_adjust: Compute inflation-adjusted value between two years

Pure stdlib — no pip dependencies required.

Usage:
    python3 mcp/external_data_server.py
"""
...
# ---------------------------------------------------------------------------
# CPI-U Data (Bureau of Labor Statistics, 1982-84=100, NSA)
# ---------------------------------------------------------------------------
...
# USD per 1 DEM (Deutsche Mark, pre-Euro)
USD_DEM = {
...
def lookup_cpi(args):
    """Look up CPI-U value for a year and optional month."""
    year = args.get("year")
    month = args.get("month")

    if not year:
        return "Error: 'year' is required. Range: 1939-2025."
    ...
    if month:
        ...
        if monthly and month in monthly:
            return (f"CPI-U for {year}-{month:02d}: {monthly[month]}\n"
                    f"(BLS, U.S. City Average, 1982-84=100, NSA)")
        elif year in CPI_ANNUAL:
            return (f"No monthly data for {year}-{month:02d}.\n"
                    f"Annual average CPI-U for {year}: {CPI_ANNUAL[year]}\n"
                    f"Monthly data available for: {sorted(CPI_MONTHLY.keys())}")
        else:
    ...
    if year in CPI_ANNUAL:
        ...
        result += "(BLS, U.S. City Average, 1982-84=100, NSA)\n"
        if year in CPI_MONTHLY:
            monthly = CPI_MONTHLY[year]
            result += f"\nMonthly values also available: {', '.join(f'{m}={v}' for m, v in sorted(monthly.items()))}"
        return result

    return f"No CPI data for year {year}. Range: 1939-2025."


def lookup_exchange_rate(args):
    """Look up exchange rate for a currency pair and year."""
    pair = args.get("pair", "").upper().replace(" ", "")
    year = args.get("year")

    if not pair:
        available = list(EXCHANGE_RATES.keys())
        return f"Error: 'pair' is required. Available pairs: {available}"
    ...
    normalized = aliases.get(pair, pair)
    rate_info = EXCHANGE_RATES.get(normalized)

    if not rate_info:
        available = list(EXCHANGE_RATES.keys())
        return f"Unknown currency pair: '{pair}'. Available: {available}"
    ...
    if year:
        ...
        if year in data:
            return (f"{normalized} for {year}: {data[year]}\n"
                    f"({rate_info['description']} — {rate_info['direction']})")
        ...
        return (f"No exact rate for {normalized} in {year}.\n"
                f"Nearest available: {', '.join(nearest)}\n"
                f"Available years: {years}")

    # Return all data for the pair
    lines = [f"{normalized} — {rate_info['description']} ({rate_info['direction']}):"]
    for y in sorted(data.keys()):
        lines.append(f"  {y}: {data[y]}")
    return "\n".join(lines)


def inflation_adjust(args):
    """Compute inflation-adjusted value between two years."""
    value = args.get("value")
    from_year = args.get("from_year")
    to_year = args.get("to_year")

    if not all([value, from_year, to_year]):
        return "Error: 'value', 'from_year', and 'to_year' are all required."
    ...
    return (f"${value:,.2f} in {from_year} dollars = ${adjusted:,.2f} in {to_year} dollars\n"
            f"Formula: {value} × ({to_cpi} / {from_cpi}) = {adjusted:.2f}\n"
            f"CPI-U {from_year}: {from_cpi}, CPI-U {to_year}: {to_cpi}")


# ---------------------------------------------------------------------------
# MCP Protocol (same pattern as corpus_index_server.py)
# ---------------------------------------------------------------------------

TOOLS = [
    {
        "name": "lookup_cpi",
        "description": "Look up CPI-U (Consumer Price Index, U.S. City Average, 1982-84=100, Not Seasonally Adjusted) for a given year. Monthly data available for selected years (1946-1949, 1969, 1979-1981). Use this for inflation calculations.",
        "inputSchema": {
            "type": "object",
            "properties": {
                "year": {"type": "integer", "description": "Year (1939-2025)"},
                "month": {"type": "integer", "description": "Month (1-12), optional. Monthly data only for: 1946-1949, 1969, 1979-1981"}
            },
            "required": ["year"]
        }
    },
    {
        ...
        "description": "Look up historical exchange rate for a currency pair and year. Available pairs: USD/GBP, USD/DEM (Deutsche Mark), INR/USD, JPY/USD, CAD/USD. Returns annual averages.",
        "inputSchema": {
            "type": "object",
            "properties": {
                "pair": {"type": "string", "description": "Currency pair (e.g., 'USD/GBP', 'JPY/USD', or just 'GBP', 'YEN')"},
                "year": {"type": "integer", "description": "Year. If omitted, returns all available years."}
            },
            "required": ["pair"]
        }
    },
    {
        ...
        "description": "Compute inflation-adjusted value from one year to another using CPI-U. Formula: value × (CPI_to / CPI_from).",
        "inputSchema": {
            "type": "object",
            "properties": {
                "value": {"type": "number", "description": "Nominal dollar amount"},
                "from_year": {"type": "integer", "description": "Year of the original value"},
                "to_year": {"type": "integer", "description": "Target year for adjustment"}
            },
            "required": ["value", "from_year", "to_year"]
        }
    }
]
